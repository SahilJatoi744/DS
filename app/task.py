...
if uploaded_file is not None:
    ...
    df.dropna(subset=['iso_code', 'continent', 'location', 'date', 'total_cases', 'new_cases',
                      'new_cases_smoothed', 'total_deaths', 'new_deaths',
                      'new_deaths_smoothed', 'reproduction_rate', 'icu_patients',
                      'hosp_patients', 'total_tests', 'new_tests', 'total_tests_per_thousand',
                      ...
                      'human_development_index'], inplace=True)
    ...
    st.write("Box Plot", sns.boxplot(df.iloc[:, 4], color='g'))
    corr = df.corr(method="pearson")
    st.write("Correlation", corr)
    st.write("Heatmap", sns.heatmap(corr, annot=True))
    fig = px.scatter(df, x='total_cases', y='life_expectancy', size='total_deaths', color='continent', hover_name='continent',
                     log_x=True, animation_frame='date', animation_group='continent')
